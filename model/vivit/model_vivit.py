...
from social_interactions.model.resse import ResNetSE
from social_interactions.model.resnet import resnet18
...
class VisionEncoder(nn.Module):
    ...
    def __init__(self, pretrained_model='vit_large_r50_s32_384', feature_dim=256):
...
class ViViT(nn.Module):
    def __init__(self, image_size=384, patch_size=16, num_classes=2, num_frames=36, dim = 512, depth = 4, heads = 3, pool = 'cls', in_channels = 3, dim_head = 64, dropout = 0.,
...
if __name__ == "__main__":
    ...
    img = torch.rand([1, 16, 3, 384, 384]).to(device)
    audio = torch.rand([16, 2]).to(device)
    
    model = ViViT(384, 16, 2, 16).to(device)
